...
if (isset($_POST['update_status'])) {
    ...
    // Validasi & Eksekusi Potong Stok saat masuk antrean "Diproses"
    if ($statusBaru === 'Diproses') {
        $q = mysqli_query($koneksi, "SELECT td.produk_id, td.jumlah, p.namaproduk FROM transaksidetail td JOIN produk p ON td.produk_id = p.id WHERE td.transaksi_id=$id");
        
        while ($item = mysqli_fetch_assoc($q)) {
            // Potong stok hanya jika stok mencukupi
            mysqli_query($koneksi, "UPDATE produk SET stok = stok - {$item['jumlah']} WHERE id={$item['produk_id']} AND stok >= {$item['jumlah']}");
            
            // Jika baris database tidak berubah, berarti stok di Laragon tidak cukup
            if (mysqli_affected_rows($koneksi) === 0) {
                echo "<script>alert('Gagal! Stok untuk buku [{$item['namaproduk']}] tidak mencukupi di gudang.'); history.back();</script>";
                exit;
            }
        }
    }
    
    // Kembalikan stok jika dibatalkan di tengah jalan
    ...
    echo "<script>alert('Status berhasil diperbarui menjadi: $statusBaru'); location='index.php?page=transaksidetail&id=$id';</script>"; 
    exit;
}
...
// Warna status — bg solid, teks kontras
$statusStyle = [
